...
client.on("messageCreate", async message => {
    ...
    try {
        ...
        if (command === "embedwithbuttons") {
            ...
            if (isNaN(count) || count < 1 || count > 5) return message.reply("لازم رقم من 1 لـ 5")
            ...
            for (let i = 1; i <= count; i++) {
                ...
                buttons.push(new ButtonBuilder().setLabel(label).setStyle(ButtonStyle.Link).setURL(link))
            }

            const row = new ActionRowBuilder().addComponents(buttons)
            const mainEmbed = new EmbedBuilder().setTitle(title).setDescription(description).setColor("#7c4dff").setTimestamp()
        ...
        if (command === "editembed") {
            ...
            const newEmbed = new EmbedBuilder().setTitle(newTitle).setDescription(newDesc).setColor("#ff4d6d").setTimestamp()
